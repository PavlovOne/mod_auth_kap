 ...
 */
int kap_proto_authorization_request_post_preserve(request_rec *r,
	...
	for (i = 0; i < arr->nelts; i++) {
		json = apr_psprintf(r->pool, "%s'%s': '%s'%s", json,
				kap_util_html_escape(r->pool, elts[i].key),
				kap_util_html_escape(r->pool, elts[i].val),
				i < arr->nelts - 1 ? "," : "");
	}
	...
	char *java_script =
			apr_psprintf(r->pool,
					...
					"        localStorage.setItem('mod_auth_kap_preserve_post_params', JSON.stringify(%s));\n"
					"        window.location='%s';\n"
					"      }\n"
					"    </script>\n", json, authorization_request);

	return kap_util_html_send(r, "Preserving...", java_script,
 ...
 */
int kap_proto_authorization_request(request_rec *r,
		struct kap_provider_t *provider, const char *login_hint,
	...
	kap_debug(r, "enter, issuer=%s, redirect_uri=%s, state=%s, proto_state=%s",
	...
	if (apr_strnatcmp(
			...
			"form_post") == 0)
		return kap_proto_authorization_request_post_preserve(r,
				authorization_request);
	...
	/* some more logging */
	kap_debug(r, "adding outgoing header: Location: %s",
			authorization_request);
 ...
 */
apr_byte_t kap_proto_is_post_authorization_response(request_rec *r,
		kap_cfg *cfg) {
 ...
 */
apr_byte_t kap_proto_is_redirect_authorization_response(request_rec *r,
		kap_cfg *cfg) {
	...
	return ((r->method_number == M_GET)
			&& kap_util_request_has_parameter(r, "state")
			&& (kap_util_request_has_parameter(r, "id_token")
					|| kap_util_request_has_parameter(r, "code")));
}

#define KAP_PROTO_NONCE_LENGTH 32
...
apr_byte_t kap_proto_generate_nonce(request_rec *r, char **nonce) {
	unsigned char *nonce_bytes = (unsigned char *)apr_pcalloc(r->pool, KAP_PROTO_NONCE_LENGTH);
	if (apr_generate_random_bytes(nonce_bytes,
			KAP_PROTO_NONCE_LENGTH) != APR_SUCCESS) {
		kap_error(r, "apr_generate_random_bytes returned an error");
		return FALSE;
	}
	if (kap_base64url_encode(r, nonce, (const char *) nonce_bytes,
			KAP_PROTO_NONCE_LENGTH, TRUE) <= 0) {
		kap_error(r, "kap_base64url_encode returned an error");
		return FALSE;
	}
	return TRUE;
}
...
apr_byte_t kap_proto_validate_nonce(request_rec *r, kap_cfg *cfg,
		kap_provider_t *provider, const char *nonce, apr_jwt_t *jwt) {

	apr_jwt_error_t err;

	/* see if we have this nonce cached already */
	const char *replay = NULL;
	cfg->cache->get(r, KAP_CACHE_SECTION_NONCE, nonce, &replay);
	if (replay != NULL) {
		kap_error(r,
	...
	cfg->cache->set(r, KAP_CACHE_SECTION_NONCE, nonce, nonce,
 ...
 */
static apr_byte_t kap_proto_validate_aud_and_azp(request_rec *r, kap_cfg *cfg,
		kap_provider_t *provider, apr_jwt_payload_t *id_token_payload) {
	...
	if (aud != NULL) {
		...
		if (json_is_string(aud)) {
			...
			if (kap_util_json_array_has_value(r, aud,
		...
		kap_error(r, "id_token JSON payload did not contain an \"aud\" claim");
		return FALSE;
	}
 ...
 */
static apr_byte_t kap_proto_validate_iat(request_rec *r, apr_jwt_t *jwt,
	...
	if (jwt->payload.iat == APR_JWT_CLAIM_TIME_EMPTY) {
		if (is_mandatory) {
			kap_error(r, "JWT did not contain an \"iat\" number value");
			return FALSE;
		}
		return TRUE;
	}
 ...
 */
static apr_byte_t kap_proto_validate_exp(request_rec *r, apr_jwt_t *jwt,
	...
	if (jwt->payload.exp == APR_JWT_CLAIM_TIME_EMPTY) {
		if (is_mandatory) {
			kap_error(r, "JWT did not contain an \"exp\" number value");
			return FALSE;
		}
		return TRUE;
	}
 ...
 */
apr_byte_t kap_proto_validate_jwt(request_rec *r, apr_jwt_t *jwt,
	...
	if (iss != NULL) {
		...
		if (kap_util_issuer_match(iss, jwt->payload.iss) == FALSE) {
			kap_error(r,
	...
	/* check exp */
	if (kap_proto_validate_exp(r, jwt, exp_is_mandatory) == FALSE)
		return FALSE;

	/* check iat */
	if (kap_proto_validate_iat(r, jwt, iat_is_mandatory, iat_slack) == FALSE)
		return FALSE;
 ...
 */
static apr_byte_t kap_proto_validate_idtoken(request_rec *r,
		kap_provider_t *provider, apr_jwt_t *jwt, const char *nonce) {

	kap_cfg *cfg = (kap_cfg *)ap_get_module_config(r->server->module_config,
			&auth_kap_module);

	kap_debug(r, "enter, jwt.header=\"%s\", jwt.payload=\"%s\", nonce=%s",
	...
	if (nonce != NULL) {
		...
		if (kap_proto_validate_nonce(r, cfg, provider, nonce, jwt) == FALSE)
			return FALSE;
	}

	/* validate the ID Token JWT, requiring iss match, and valid exp + iat */
	if (kap_proto_validate_jwt(r, jwt, provider->issuer, TRUE, TRUE,
	...
	if (kap_proto_validate_aud_and_azp(r, cfg, provider,
			&jwt->payload) == FALSE)
		return FALSE;
 ...
 */
static apr_byte_t kap_proto_get_key_from_jwks(request_rec *r, apr_jwt_t *jwt,
	...
	if (key_type == NULL) {
		kap_error(r, "unsupported signing algorithm in JWT header: %s",
				jwt->header.alg);
		return FALSE;
	}
	...
	for (i = 0; i < json_array_size(keys); i++) {
		...
		if ((jwt->header.kid == NULL) && (x5t == NULL)) {
			kap_debug(r, "no kid/x5t to match, include matching key type");

			rc = apr_jwk_parse_json(r->pool, elem, &jwk, &err);
			if (rc == FALSE)
				kap_error(r, "JWK parsing failed: %s",
						apr_jwt_e2s(r->pool, err));
			else if (jwk->kid)
		...
		if ((ekid != NULL) && json_is_string(ekid)
			...
			if (apr_strnatcmp(jwt->header.kid, json_string_value(ekid)) == 0) {
				kap_debug(r, "found matching kid: \"%s\"", jwt->header.kid);
		...
		if ((ex5t != NULL) && json_is_string(ex5t) && (x5t != NULL)) {
			...
			if (apr_strnatcmp(x5t, json_string_value(ex5t)) == 0) {
				kap_debug(r, "found matching x5t: \"%s\"", x5t);
 ...
 */
apr_byte_t kap_proto_get_keys_from_jwks_uri(request_rec *r, kap_cfg *cfg,
		apr_jwt_t *jwt, const kap_jwks_uri_t *jwks_uri, apr_hash_t *keys,
	...
	kap_metadata_jwks_get(r, cfg, jwks_uri, &j_jwks, force_refresh);
	if (j_jwks == NULL) {
		kap_error(r, "could not %s JSON Web Keys",
	 ...
	 * already done
	 */
	kap_proto_get_key_from_jwks(r, jwt, j_jwks, keys);
	...
	if ((apr_hash_count(keys) < 1) && (*force_refresh == FALSE)) {
		...
		*force_refresh = TRUE;
		return kap_proto_get_keys_from_jwks_uri(r, cfg, jwt, jwks_uri, keys,
				force_refresh);
	}
 ...
 */
apr_byte_t kap_proto_jwt_verify(request_rec *r, kap_cfg *cfg, apr_jwt_t *jwt,
		const kap_jwks_uri_t *jwks_uri, apr_hash_t *static_keys) {
	...
	if (jwks_uri->url == NULL) {
		...
		/* get the key from the JWKs that corresponds with the key specified in the header */
		if (kap_proto_get_keys_from_jwks_uri(r, cfg, jwt, jwks_uri,
	...
	if (apr_jws_verify(r->pool, jwt,
			kap_util_merge_key_sets(r->pool, static_keys, dynamic_keys),
			&err) == FALSE) {
		kap_error(r, "JWT signature verification failed: %s",
				apr_jwt_e2s(r->pool, err));
		return FALSE;
	}
 ...
 */
apr_byte_t kap_proto_parse_idtoken(request_rec *r, kap_cfg *cfg,
		kap_provider_t *provider, const char *id_token, const char *nonce,
	...
	if (apr_jwt_parse(r->pool, id_token, jwt,
			kap_util_merge_symmetric_key(r->pool, cfg->private_keys,
					provider->client_secret, "sha256"), &err) == FALSE) {
		kap_error(r, "apr_jwt_parse failed for JWT with header \"%s\": %s",
	...
	if (is_code_flow == FALSE || strcmp((*jwt)->header.alg, "none") != 0) {
		...
		if (kap_proto_jwt_verify(r, cfg, *jwt, &jwks_uri,
	...
	if (kap_proto_validate_idtoken(r, provider, *jwt, nonce) == FALSE) {
		kap_error(r, "id_token payload could not be validated, aborting");
		apr_jwt_destroy(*jwt);
		*jwt = NULL;
		return FALSE;
	}
 ...
 */
static apr_byte_t kap_proto_validate_token_type(request_rec *r,
		kap_provider_t *provider, const char *token_type) {
 ...
 */
static apr_byte_t kap_proto_token_endpoint_request(request_rec *r,
		kap_cfg *cfg, kap_provider_t *provider, apr_table_t *params,
	...
	json_t *result = NULL;
	if (kap_util_decode_json_and_check_error(r, response, &result) == FALSE)
		return FALSE;

	/* get the id_token from the parsed response */
	kap_json_object_get_string(r->pool, result, "id_token", id_token, NULL);

	/* get the access_token from the parsed response */
	kap_json_object_get_string(r->pool, result, "access_token", access_token,
			NULL);

	/* get the token type from the parsed response */
	kap_json_object_get_string(r->pool, result, "token_type", token_type,
			NULL);

	/* check the new token type */
	if (token_type != NULL) {
		if (kap_proto_validate_token_type(r, provider, *token_type) == FALSE) {
			kap_warn(r, "access token type did not validate, dropping it");
			*access_token = NULL;
		}
	}

	/* get the expires_in value */
	kap_json_object_get_int(r->pool, result, "expires_in", expires_in, -1);

	/* get the refresh_token from the parsed response */
	kap_json_object_get_string(r->pool, result, "refresh_token", refresh_token,
			NULL);
 ...
 */
apr_byte_t kap_proto_resolve_code(request_rec *r, kap_cfg *cfg,
		kap_provider_t *provider, const char *code, char **id_token,
	...
	return kap_proto_token_endpoint_request(r, cfg, provider, params, id_token,
 ...
 */
apr_byte_t kap_proto_refresh_request(request_rec *r, kap_cfg *cfg,
		kap_provider_t *provider, const char *rtoken, char **id_token,
		char **access_token, char **token_type, int *expires_in,
		char **refresh_token) {
 ...
 */
apr_byte_t kap_proto_resolve_userinfo(request_rec *r, kap_cfg *cfg,
		kap_provider_t *provider, const char *access_token,
		const char **response) {
	...
	kap_debug(r, "enter, endpoint=%s, access_token=%s",
			provider->userinfo_endpoint_url, access_token);

	/* get the JSON response */
	if (kap_util_http_get(r, provider->userinfo_endpoint_url,
	...
	json_t *claims = NULL;
	if (kap_util_decode_json_and_check_error(r, *response, &claims) == FALSE)
		return FALSE;
	json_decref(claims);
 ...
 */
apr_byte_t kap_proto_account_based_discovery(request_rec *r, kap_cfg *cfg,
	...
	const char *response = NULL;
	if (kap_util_http_get(r, url, params, NULL, NULL,
	...
	if (kap_util_decode_json_and_check_error(r, response, &j_response) == FALSE)
		return FALSE;

	/* get the links parameter */
	json_t *j_links = json_object_get(j_response, "links");
	if ((j_links == NULL) || (!json_is_array(j_links))) {
		kap_error(r, "response JSON object did not contain a \"links\" array");
...
int kap_proto_javascript_implicit(request_rec *r, kap_cfg *c) {
	...
	return kap_util_html_send(r, "Submitting...", java_script, "postOnLoad",
			html_body, DONE);
}
 ...
 */
static apr_byte_t kap_proto_validate_hash(request_rec *r, const char *alg,
	...
	if (apr_jws_hash_string(r->pool, alg, value, &calc, &calc_len,
			&err) == FALSE) {
		kap_error(r, "apr_jws_hash_string failed: %s",
				apr_jwt_e2s(r->pool, err));
		return FALSE;
	}
	...
	char *decoded = NULL;
	unsigned int decoded_len = kap_base64url_decode(r, &decoded, hash);
	if (decoded_len <= 0) {
		kap_error(r, "kap_base64url_decode returned an error");
		return FALSE;
	}

	kap_debug(r, "hash_len=%d, decoded_len=%d, calc_len=%d", hash_len,
 ...
 */
static apr_byte_t kap_proto_validate_hash_value(request_rec *r,
		kap_provider_t *provider, apr_jwt_t *jwt, const char *response_type,
	...
	if (hash == NULL) {
		...
		for (i = 0; i < required_for_flows->nelts; i++) {
			if (kap_util_spaced_string_equals(r->pool, response_type,
					((const char**) required_for_flows->elts)[i])) {
				kap_warn(r, "flow is \"%s\", but no %s found in id_token",
						response_type, key);
				return FALSE;
			}
		}
	 ...
	 */
	return kap_proto_validate_hash(r, jwt->header.alg, hash, value, key);
}

/*
 * check the c_hash value in the id_token against the code
 */
apr_byte_t kap_proto_validate_code(request_rec *r, kap_provider_t *provider,
	...
	*(const char**) apr_array_push(required_for_flows) = "code id_token token";
	if (kap_proto_validate_hash_value(r, provider, jwt, response_type, code,
			"c_hash", required_for_flows) == FALSE) {
		kap_error(r, "could not validate code against c_hash");
		return FALSE;
	}
	return TRUE;
}
 ...
 */
apr_byte_t kap_proto_validate_access_token(request_rec *r,
		kap_provider_t *provider, apr_jwt_t *jwt, const char *response_type,
	...
	*(const char**) apr_array_push(required_for_flows) = "code id_token token";
	if (kap_proto_validate_hash_value(r, provider, jwt, response_type,
			access_token, "at_hash", required_for_flows) == FALSE) {
		kap_error(r, "could not validate access token against at_hash");
		return FALSE;
	}
	return TRUE;
}
...
apr_byte_t kap_proto_flow_is_supported(apr_pool_t *pool, const char *flow) {
	apr_array_header_t *flows = kap_proto_supported_flows(pool);
	int i;
	for (i = 0; i < flows->nelts; i++) {
		if (kap_util_spaced_string_equals(pool, flow,
 ...
 */
static apr_byte_t kap_proto_validate_code_response(request_rec *r,
	 ...
	 */
	if (!kap_util_spaced_string_contains(r->pool, response_type, "id_token")) {
		if (id_token == NULL) {
			kap_error(r,
	 ...
	 */
	if (!kap_util_spaced_string_contains(r->pool, response_type, "token")) {
 ...
 */
static apr_byte_t kap_proto_validate_response_type(request_rec *r,
		const char *requested_response_type, const char *code,
		const char *id_token, const char *access_token) {

	if (kap_util_spaced_string_contains(r->pool, requested_response_type,
			"code")) {
		if (code == NULL) {
			kap_error(r,
 ...
 */
static apr_byte_t kap_proto_validate_response_mode(request_rec *r,
 ...
 */
static apr_byte_t kap_proto_validate_response_type_and_response_mode(
	...
	if (kap_proto_validate_response_type(r, requested_response_type, code,
			id_token, access_token) == FALSE)
		return FALSE;

	if (kap_proto_validate_response_mode(r, proto_state, response_mode,
 ...
 */
static apr_byte_t kap_proto_parse_idtoken_and_validate_code(request_rec *r,
		kap_cfg *c, json_t *proto_state, kap_provider_t *provider,
	...
	apr_byte_t is_code_flow = (kap_util_spaced_string_contains(r->pool,
			response_type, "code") == TRUE)
			&& (kap_util_spaced_string_contains(r->pool, response_type,
					"id_token") == FALSE);

	json_t *nonce = json_object_get(proto_state, "nonce");

	if (kap_proto_parse_idtoken(r, c, provider, id_token,
			nonce ? json_string_value(nonce) : NULL, jwt, is_code_flow) == FALSE)
		return FALSE;

	if ((must_validate_code == TRUE)
			&& (kap_proto_validate_code(r, provider, *jwt, response_type, code)
					== FALSE))
		return FALSE;
 ...
 */
static apr_byte_t kap_proto_resolve_code_and_validate_response(request_rec *r,
		kap_cfg *c, kap_provider_t *provider, const char *response_type,
		apr_table_t *params) {
	...
	if (kap_proto_resolve_code(r, c, provider, apr_table_get(params, "code"),
			&id_token, &access_token, &token_type, &expires_in,
			&refresh_token) == FALSE) {
		kap_error(r, "failed to resolve the code");
		return FALSE;
	}

	if (kap_proto_validate_code_response(r, response_type, id_token,
			access_token, token_type) == FALSE) {
		kap_error(r, "code response validation failed");
		return FALSE;
	}
 ...
 */
apr_byte_t kap_proto_authorization_response_code_idtoken(request_rec *r,
		kap_cfg *c, json_t *proto_state, kap_provider_t *provider,
	...
	if (kap_proto_validate_response_type_and_response_mode(r, response_type,
			params, proto_state, response_mode, "fragment") == FALSE)
		return FALSE;

	if (kap_proto_parse_idtoken_and_validate_code(r, c, proto_state, provider,
	...
	if (kap_proto_resolve_code_and_validate_response(r, c, provider,
 ...
 */
apr_byte_t kap_proto_handle_authorization_response_code_token(request_rec *r,
		kap_cfg *c, json_t *proto_state, kap_provider_t *provider,
		apr_table_t *params, const char *response_mode, apr_jwt_t **jwt) {
 ...
 */
apr_byte_t kap_proto_handle_authorization_response_code(request_rec *r,
		kap_cfg *c, json_t *proto_state, kap_provider_t *provider,
		apr_table_t *params, const char *response_mode, apr_jwt_t **jwt) {
	 ...
	 */
	if ((apr_table_get(params, "access_token") != NULL)
			&& (kap_proto_validate_access_token(r, provider, *jwt,
 ...
 */
static apr_byte_t kap_proto_handle_implicit_flow(request_rec *r, kap_cfg *c,
		const char *response_type, json_t *proto_state,
		kap_provider_t *provider, apr_table_t *params,
 ...
 */
apr_byte_t kap_proto_authorization_response_code_idtoken_token(request_rec *r,
		kap_cfg *c, json_t *proto_state, kap_provider_t *provider,
		apr_table_t *params, const char *response_mode, apr_jwt_t **jwt) {
	...
	if (kap_proto_handle_implicit_flow(r, c, response_type, proto_state,
			provider, params, response_mode, jwt) == FALSE)
		return FALSE;

	if (kap_proto_validate_access_token(r, provider, *jwt, response_type,
 ...
 */
apr_byte_t kap_proto_handle_authorization_response_idtoken_token(
		request_rec *r, kap_cfg *c, json_t *proto_state,
		kap_provider_t *provider, apr_table_t *params,
		const char *response_mode, apr_jwt_t **jwt) {
 ...
 */
apr_byte_t kap_proto_handle_authorization_response_idtoken(request_rec *r,
		kap_cfg *c, json_t *proto_state, kap_provider_t *provider,
		apr_table_t *params, const char *response_mode, apr_jwt_t **jwt) {
