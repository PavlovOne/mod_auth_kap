...
#define KAP_SESSION_REMOTE_USER_KEY "remote-user"
/* the name of the session expiry attribute in the session */
#define KAP_SESSION_EXPIRY_KEY      "kap-expiry"
/* the name of the uuid attribute in the session */
#define KAP_SESSION_UUID_KEY        "kap-uuid"
...
apr_status_t kap_session_load(request_rec *r, session_rec **zz) {
	apr_status_t rc = ap_session_load_fn(r, zz);
	(*zz)->remote_user = apr_table_get((*zz)->entries,
			KAP_SESSION_REMOTE_USER_KEY);
	const char *uuid = apr_table_get((*zz)->entries, KAP_SESSION_UUID_KEY);
	kap_debug(r, "%s", uuid ? uuid : "<null>");
	if (uuid != NULL)
...
apr_status_t kap_session_save(request_rec *r, session_rec *z) {
	kap_session_set(r, z, KAP_SESSION_REMOTE_USER_KEY, z->remote_user);
	char key[APR_UUID_FORMATTED_LENGTH + 1];
	apr_uuid_format((char *) &key, z->uuid);
	kap_debug(r, "%s", key);
	kap_session_set(r, z, KAP_SESSION_UUID_KEY, key);
	return ap_session_save_fn(r, z);
}

apr_status_t kap_session_get(request_rec *r, session_rec *z, const char *key,
		const char **value) {
	return ap_session_get_fn(r, z, key, value);
}

apr_status_t kap_session_set(request_rec *r, session_rec *z, const char *key,
		const char *value) {
...
#ifndef KAP_SESSION_USE_APACHE_SESSIONS
...
static int kap_session_unescape_url(char *url, const char *forbid,
		const char *reserved) {
...
AP_DECLARE(int) ap_unescape_urlencoded(char *query) {
	...
	return kap_session_unescape_url(query, NULL, NULL);
}
#endif

// copied from mod_session.c
static apr_status_t kap_session_identity_decode(request_rec * r,
		session_rec * z) {
	char *last = NULL;
	char *encoded, *pair;
	const char *sep = "&";

	//kap_debug(r, "decoding %s", z->encoded);
	...
	while (pair && pair[0]) {
		...
		//kap_debug(r, "decoding %s=%s", key, val);
		...
		if (key && *key) {
			if (!val || !*val) {
				...
				if (!strcmp(KAP_SESSION_EXPIRY_KEY, key)) {
...
// copied from mod_session.c
static int kap_identity_count(int *count, const char *key, const char *val) {
	*count += (int)strlen(key) * 3 + (int)strlen(val) * 3 + 1;
	return 1;
}

// copied from mod_session.c
static int kap_identity_concat(char *buffer, const char *key, const char *val) {
	char *slider = buffer;
...
// copied from mod_session.c
static apr_status_t kap_session_identity_encode(request_rec * r,
		session_rec * z) {
	char *buffer = NULL;
	int length = 0;
	if (z->expiry) {
		char *expiry = apr_psprintf(z->pool, "%" APR_INT64_T_FMT, z->expiry);
		apr_table_setn(z->entries, KAP_SESSION_EXPIRY_KEY, expiry);
	}
	apr_table_do(
			(int (*)(void *, const char *, const char *)) kap_identity_count,
			&length, z->entries, NULL);
	buffer = (char *)apr_pcalloc(r->pool, length + 1);
	apr_table_do(
			(int (*)(void *, const char *, const char *)) kap_identity_concat,
...
static apr_status_t kap_session_load_cache(request_rec *r, session_rec *z) {
	...
	char *uuid = kap_util_get_cookie(r, d->cookie);

	/* get the string-encoded session from the cache based on the key */
	if (uuid != NULL) {
		c->cache->get(r, KAP_CACHE_SECTION_SESSION, uuid, &z->encoded);
		//kap_util_set_cookie(r, d->cookie, "");
	}
 ...
 */
static apr_status_t kap_session_save_cache(request_rec *r, session_rec *z) {
	kap_cfg *c = (kap_cfg *)ap_get_module_config(r->server->module_config,
			&auth_kap_module);
	kap_dir_cfg *d = (kap_dir_cfg *)ap_get_module_config(r->per_dir_config,
			&auth_kap_module);
	...
	if (z->encoded && z->encoded[0]) {
		...
		kap_util_set_cookie(r, d->cookie, key, -1);

		/* store the string-encoded session in the cache */
		c->cache->set(r, KAP_CACHE_SECTION_SESSION, key, z->encoded,
				z->expiry);

	} else {

		/* clear the cookie */
		kap_util_set_cookie(r, d->cookie, "", 0);

		/* remove the session from the cache */
		c->cache->set(r, KAP_CACHE_SECTION_SESSION, key, NULL, 0);
	}

	return APR_SUCCESS;
}

static apr_status_t kap_session_load_cookie(request_rec *r, session_rec *z) {
	kap_dir_cfg *d = (kap_dir_cfg *)ap_get_module_config(r->per_dir_config,
			&auth_kap_module);

	char *cookieValue = kap_util_get_cookie(r, d->cookie);
	if (cookieValue != NULL) {
		if (kap_base64url_decode_decrypt_string(r, (char **) &z->encoded,
				cookieValue) <= 0) {
			//kap_util_set_cookie(r, d->cookie, "");
			kap_warn(r, "cookie value possibly corrupted");
			return APR_EGENERAL;
		}
	}
...
static apr_status_t kap_session_save_cookie(request_rec *r, session_rec *z) {
	kap_dir_cfg *d = (kap_dir_cfg *)ap_get_module_config(r->per_dir_config,
			&auth_kap_module);

	char *cookieValue = "";
	if (z->encoded && z->encoded[0]) {
		if (kap_encrypt_base64url_encode_string(r, &cookieValue, z->encoded) <= 0) {
			kap_error(r, "kap_encrypt_base64url_encode_string failed");
			return APR_EGENERAL;
		}
	}
	kap_util_set_cookie(r, d->cookie, cookieValue, -1);
 ...
 */
static apr_status_t kap_session_load_22(request_rec *r, session_rec **zz) {

	kap_cfg *c = (kap_cfg *)ap_get_module_config(r->server->module_config,
			&auth_kap_module);

	/* first see if this is a sub-request and it was set already in the main request */
	if (((*zz) = (session_rec *) kap_request_state_get(r, "session")) != NULL) {
		kap_debug(r, "loading session from request state");
		return APR_SUCCESS;
	}
	...
	if (c->session_type == KAP_SESSION_TYPE_22_SERVER_CACHE) {
		/* load the session from the cache */
		rc = kap_session_load_cache(r, z);
	} else if (c->session_type == KAP_SESSION_TYPE_22_CLIENT_COOKIE) {
		/* load the session from a self-contained cookie */
		rc = kap_session_load_cookie(r, z);
	} else {
		kap_error(r, "kap_session_load_22: unknown session type: %d",
				c->session_type);
		rc = APR_EGENERAL;
	}
	...
	if (kap_session_identity_decode(r, z) != APR_SUCCESS)
		return APR_EGENERAL;

	/* check whether it has expired */
	if (apr_time_now() > z->expiry) {

		kap_warn(r, "session restored from cache has expired");
		apr_table_clear(z->entries);
		z->expiry = 0;
		z->encoded = NULL;
	...
	kap_request_state_set(r, "session", (const char *) z);
 ...
 */
static apr_status_t kap_session_save_22(request_rec *r, session_rec *z) {

	kap_cfg *c = (kap_cfg *)ap_get_module_config(r->server->module_config,
			&auth_kap_module);

	/* encode the actual state in to the encoded string */
	kap_session_identity_encode(r, z);
	...
	if (c->session_type == KAP_SESSION_TYPE_22_SERVER_CACHE) {
		...
		rc = kap_session_save_cache(r, z);
	} else if (c->session_type == KAP_SESSION_TYPE_22_CLIENT_COOKIE) {
		/* store the session in a self-contained cookie */
		rc = kap_session_save_cookie(r, z);
	} else {
		kap_error(r, "unknown session type: %d", c->session_type);
		rc = APR_EGENERAL;
	}
 ...
 */
static apr_status_t kap_session_get_22(request_rec *r, session_rec *z,
 ...
 */
static apr_status_t kap_session_set_22(request_rec *r, session_rec *z,
		const char *key, const char *value) {
...
apr_status_t kap_session_init() {
	if (!ap_session_load_fn || !ap_session_get_fn || !ap_session_set_fn
			|| !ap_session_save_fn) {
		ap_session_load_fn = kap_session_load_22;
		ap_session_get_fn = kap_session_get_22;
		ap_session_set_fn = kap_session_set_22;
		ap_session_save_fn = kap_session_save_22;
	}
	return OK;
}
