...
static apr_byte_t apr_jwk_parse_rsa_raw(apr_pool_t *pool, json_t *json,
	...
	/* allocate space */
	*jwk_key_rsa = (apr_jwk_key_rsa_t *)apr_pcalloc(pool, sizeof(apr_jwk_key_rsa_t));
...
static apr_byte_t apr_jwk_rsa_bio_to_key(apr_pool_t *pool, BIO *input,
	...
	*jwk_key_rsa = (apr_jwk_key_rsa_t *)apr_pcalloc(pool, sizeof(apr_jwk_key_rsa_t));
	...
	key->modulus = (unsigned char *)apr_pcalloc(pool, key->modulus_len);
	BN_bn2bin(rsa->n, key->modulus);

	/* convert the exponent bignum in to a key/len */
	key->exponent_len = BN_num_bytes(rsa->e);
	key->exponent = (unsigned char *)apr_pcalloc(pool, key->exponent_len);
	...
	if (rsa->d != NULL) {
		...
		key->private_exponent = (unsigned char *)apr_pcalloc(pool, key->private_exponent_len);
...
static apr_byte_t apr_jwk_parse_ec(apr_pool_t *pool, json_t *json,
	...
	jwk->key.ec = (apr_jwk_key_ec_t *)apr_pcalloc(pool, sizeof(apr_jwk_key_ec_t));
...
static apr_byte_t apr_jwk_parse_oct(apr_pool_t *pool, json_t *json,
	...
	jwk->key.oct = (apr_jwk_key_oct_t *)apr_pcalloc(pool, sizeof(apr_jwk_key_oct_t));
...
apr_byte_t apr_jwk_parse_symmetric_key(apr_pool_t *pool, const unsigned char *key,
	...
	*j_jwk = (apr_jwk_t *)apr_pcalloc(pool, sizeof(apr_jwk_t));
	apr_jwk_t *jwk = *j_jwk;
	...
	//	/* set the values */
	jwk->key.oct->k = (unsigned char *)apr_pcalloc(pool, key_len);
