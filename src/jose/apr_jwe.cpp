...
static apr_array_header_t *apr_jwe_unpacked_base64url_decode(apr_pool_t *pool,
	...
	for (i = 0; i < unpacked->nelts; i++) {
		apr_jwe_unpacked_t *elem = (apr_jwe_unpacked_t *)apr_pcalloc(pool,
...
static apr_byte_t apr_jwe_decrypt_cek_rsa(apr_pool_t *pool,
	...
	*cek = (unsigned char *)apr_pcalloc(pool, RSA_size(pkey));
...
static apr_byte_t apr_jwe_decrypt_cek_oct_aes(apr_pool_t *pool,
	...
	*cek_len = (apr_strnatcmp(header->enc, "A128CBC-HS256") == 0) ? 32 : 64;
	...
	/* unwrap the AES key */
	*cek = (unsigned char *)apr_pcalloc(pool, *cek_len);
...
static apr_byte_t apr_jwe_decrypt_cek(apr_pool_t *pool,
	...
	if (header->kid != NULL) {
		...
		jwk = (apr_jwk_t *)apr_hash_get(keys, header->kid,
			APR_HASH_KEY_STRING);
		if (jwk != NULL) {
...
apr_byte_t apr_jwe_decrypt_jwt(apr_pool_t *pool, apr_jwt_header_t *header,
	...
	unsigned char *mac_key = (unsigned char *)apr_pcalloc(pool, cek_len / 2);
	memcpy(mac_key, cek, cek_len / 2);
	/* extract encryption key from CEK: first half of CEK bits */
	unsigned char *enc_key = (unsigned char *)apr_pcalloc(pool, cek_len / 2);
	...
	const unsigned char *msg = (const unsigned char *)apr_pcalloc(pool, msg_len);
	char *p = (char*) msg;
	...
	unsigned char *plaintext = (unsigned char *)apr_palloc(pool, p_len + AES_BLOCK_SIZE);
