 ...
 */
int kap_base64url_encode(request_rec *r, char **dst, const char *src,
		int src_len, int remove_padding) {
	if ((src == NULL) || (src_len <= 0)) {
		kap_error(r, "not encoding anything; src=NULL and/or src_len<1");
		return -1;
	}
...
int kap_base64url_decode(request_rec *r, char **dst, const char *src) {
	if (src == NULL) {
		kap_error(r, "not decoding anything; src=NULL");
		return -1;
	}
 ...
 */
int kap_encrypt_base64url_encode_string(request_rec *r, char **dst,
		const char *src) {
	kap_cfg *c = (kap_cfg *)ap_get_module_config(r->server->module_config,
			&auth_kap_module);
	int crypted_len = (int)strlen(src) + 1;
	unsigned char *crypted = kap_crypto_aes_encrypt(r, c,
			(unsigned char *) src, &crypted_len);
	if (crypted == NULL) {
		kap_error(r, "kap_crypto_aes_encrypt failed");
		return -1;
	}
	return kap_base64url_encode(r, dst, (const char *) crypted, crypted_len, 1);
}

/*
 * decrypt and base64url decode a string
 */
int kap_base64url_decode_decrypt_string(request_rec *r, char **dst,
		const char *src) {
	kap_cfg *c = (kap_cfg *)ap_get_module_config(r->server->module_config,
			&auth_kap_module);
	char *decbuf = NULL;
	int dec_len = kap_base64url_decode(r, &decbuf, src);
	if (dec_len <= 0) {
		kap_error(r, "kap_base64url_decode failed");
		return -1;
	}
	*dst = (char *) kap_crypto_aes_decrypt(r, c, (unsigned char *) decbuf,
			&dec_len);
	if (*dst == NULL) {
		kap_error(r, "kap_crypto_aes_decrypt failed");
		return -1;
	}
	return dec_len;
}
...
int kap_strnenvcmp(const char *a, const char *b, int len) {
	...
	while (1) {
		...
		d = kap_char_to_env(*a) - kap_char_to_env(*b);
		if (d)
			return d;
...
char *kap_util_escape_string(const request_rec *r, const char *str) {
	...
	if (curl == NULL) {
		kap_error(r, "curl_easy_init() error");
		return NULL;
	}
	char *result = curl_easy_escape(curl, str, 0);
	if (result == NULL) {
		kap_error(r, "curl_easy_escape() error");
		return NULL;
	}
...
char *kap_util_unescape_string(const request_rec *r, const char *str) {
	...
	if (result == NULL) {
		kap_error(r, "curl_easy_unescape() error");
		return NULL;
	}
	char *rv = apr_pstrdup(r->pool, result);
	curl_free(result);
	curl_easy_cleanup(curl);
	//kap_debug(r, "input=\"%s\", output=\"%s\"", str, rv);
	return rv;
}
 ...
 */
char *kap_util_html_escape(apr_pool_t *pool, const char *s) {
 ...
 */
static const char *kap_get_current_url_scheme(const request_rec *r) {
 ...
 */
static const char *kap_get_current_url_port(const request_rec *r,
		const kap_cfg *c, const char *scheme_str) {
	/* first see if there's a proxy/load-balancer in front of us */
...
char *kap_get_current_url(const request_rec *r, const kap_cfg *c) {
	...
	const char *scheme_str = kap_get_current_url_scheme(r);

	const char *port_str = kap_get_current_url_port(r, c, scheme_str);
	...
	kap_debug(r, "current URL '%s'", url);

	return url;
}

/* maximum size of any response returned in HTTP calls */
#define KAP_CURL_MAX_RESPONSE_SIZE 65536

/* buffer to hold HTTP call responses */
typedef struct kap_curl_buffer {
	char buf[KAP_CURL_MAX_RESPONSE_SIZE];
	size_t written;
} kap_curl_buffer;
 ...
 */
size_t kap_curl_write(const void *ptr, size_t size, size_t nmemb, void *stream) {
	kap_curl_buffer *curlBuffer = (kap_curl_buffer *) stream;

	if ((nmemb * size) + curlBuffer->written >= KAP_CURL_MAX_RESPONSE_SIZE)
		return 0;
...
typedef struct kap_http_encode_t {
	request_rec *r;
 ...
 */
static int kap_http_add_form_url_encoded_param(void* rec, const char* key,
		const char* value) {
	kap_http_encode_t *ctx = (kap_http_encode_t*) rec;
	const char *sep = apr_strnatcmp(ctx->encoded_params, "") == 0 ? "" : "&";
	ctx->encoded_params = apr_psprintf(ctx->r->pool, "%s%s%s=%s",
			ctx->encoded_params, sep, kap_util_escape_string(ctx->r, key),
			kap_util_escape_string(ctx->r, value));
	return 1;
}
 ...
 */
static apr_byte_t kap_util_http_call(request_rec *r, const char *url,
	...
	curl_easy_setopt(curl, CURLOPT_WRITEFUNCTION, kap_curl_write);
	...
	if (buflen > 0)
		...
		kap_warn(r, "no curl-ca-bundle.crt file found in path");
	free(retval);
#endif

	/* identify this HTTP client */
	curl_easy_setopt(curl, CURLOPT_USERAGENT, "mod_auth_kap");
	...
	for (i = 0; i < pass_cookies->nelts; i++) {
		...
		char *cookie_value = kap_util_get_cookie(r, cookie_name);
	...
	if (cookie_string != NULL) {
		kap_debug(r, "passing browser cookies on backend call: %s",
				cookie_string);
	...
	if (curl_easy_perform(curl) != CURLE_OK) {
		kap_error(r, "curl_easy_perform() failed on: %s (%s)", url, curlError);
		rv = FALSE;
		goto out;
	}
	...
	*response = apr_pstrndup(r->pool, curlBuffer.buf, curlBuffer.written);
	...
	kap_debug(r, "response=%s", *response);
 ...
 */
apr_byte_t kap_util_http_get(request_rec *r, const char *url,
	...
	if ((params != NULL) && (apr_table_elts(params)->nelts > 0)) {
		kap_http_encode_t data = { r, "" };
		apr_table_do(kap_http_add_form_url_encoded_param, &data, params, NULL);
		const char *sep = strchr(url, '?') != NULL ? "&" : "?";
		url = apr_psprintf(r->pool, "%s%s%s", url, sep, data.encoded_params);
		kap_debug(r, "get URL=\"%s\"", url);
	}

	return kap_util_http_call(r, url, NULL, NULL, basic_auth, bearer_token,
 ...
 */
apr_byte_t kap_util_http_post_form(request_rec *r, const char *url,
		const apr_table_t *params, const char *basic_auth,
		const char *bearer_token, int ssl_validate_server,
		const char **response, int timeout, const char *outgoing_proxy,
		apr_array_header_t *pass_cookies) {
	...
	if ((params != NULL) && (apr_table_elts(params)->nelts > 0)) {
		kap_http_encode_t encode_data = { r, "" };
		apr_table_do(kap_http_add_form_url_encoded_param, &encode_data, params,
				NULL);
		data = encode_data.encoded_params;
		kap_debug(r, "post data=\"%s\"", data);
	}

	return kap_util_http_call(r, url, data,
 ...
 */
apr_byte_t kap_util_http_post_json(request_rec *r, const char *url,
	...
	return kap_util_http_call(r, url, data, "application/json", basic_auth,
 ...
 */
static char *kap_util_get_path(request_rec *r) {
	size_t i;
	char *p;
	p = r->parsed_uri.path;
	if (p[0] == '\0')
 ...
 */
static char *kap_util_get_cookie_path(request_rec *r) {
	char *rv = NULL, *requestPath = kap_util_get_path(r);
	kap_dir_cfg *d = (kap_dir_cfg *)ap_get_module_config(r->per_dir_config,
			&auth_kap_module);
	if (d->cookie_path != NULL) {
		if (strncmp(d->cookie_path, requestPath, strlen(d->cookie_path)) == 0)
			...
			kap_warn(r,
					"KAPCookiePath (%s) not a substring of request path, using request path (%s) for cookie",
 ...
 */
void kap_util_set_cookie(request_rec *r, const char *cookieName,
	...
	if (expires != -1) {
		...
		if (apr_rfc822_date(expiresString, expires) != APR_SUCCESS) {
			kap_error(r, "could not set cookie expiry date");
		}
	}
	...
	/* do some logging */
	kap_debug(r, "adding outgoing header: Set-Cookie: %s", headerString);
}
...
char *kap_util_get_cookie(request_rec *r, const char *cookieName) {
	...
	kap_debug(r, "returning \"%s\" = %s", cookieName, rv ? apr_psprintf(r->pool, "\"%s\"", rv) : "<null>");
...
apr_byte_t kap_util_request_matches_url(request_rec *r, const char *url) {
	...
	kap_debug(r, "comparing \"%s\"==\"%s\"", r->parsed_uri.path, uri.path);
 ...
 */
apr_byte_t kap_util_get_request_parameter(request_rec *r, char *name,
		char **value) {
	...
	*value = NULL;
	...
	do {
		if (p && strncmp(p, k_param, k_param_sz) == 0) {
			...
			*value = kap_util_unescape_string(r, *value);
		}
 ...
 */
static apr_byte_t kap_util_json_string_print(request_rec *r, json_t *result,
	...
	if (value != NULL && !json_is_null(value)) {
		...
		kap_error(r, "%s: response contained an \"%s\" entry with value: \"%s\"",
				log, key, s_value);
		free(s_value);
		return TRUE;
	}
 ...
 */
static apr_byte_t kap_util_check_json_error(request_rec *r, json_t *json) {
	if (kap_util_json_string_print(r, json, "error",
			"kap_util_check_json_error") == TRUE) {
		kap_util_json_string_print(r, json, "error_description",
				"kap_util_check_json_error");
		return TRUE;
	}
	return FALSE;
}
 ...
 */
apr_byte_t kap_util_decode_json_and_check_error(request_rec *r,
	...
	*json = json_loads(str, 0, &json_error);
	...
	if (*json == NULL) {
		...
		kap_error(r, "JSON parsing returned an error: %s", json_error.text);
		return FALSE;
	}
	...
	if (kap_util_check_json_error(r, *json) == TRUE) {
		json_decref(*json);
		*json = NULL;
		return FALSE;
	}
 ...
 */
int kap_util_http_send(request_rec *r, const char *data, int data_len,
 ...
 */
int kap_util_html_send(request_rec *r, const char *title,
	...
	html = apr_psprintf(r->pool, html,
			title ? kap_util_html_escape(r->pool, title) : "",
			html_head ? html_head : "",
			on_load ? apr_psprintf(r->pool, " onload=\"%s()\"", on_load) : "",
			html_body ? html_body : "<p></p>");

	return kap_util_http_send(r, html, (int)strlen(html), "text/html", status_code);
}

/*
 * send a user-facing error to the browser
 */
int kap_util_html_send_error(request_rec *r, const char *error,
	...
	if (error != NULL) {
		html_body = apr_psprintf(r->pool, "%s<p>Error: <pre>%s</pre></p>",
				html_body, kap_util_html_escape(r->pool, error));
	}
	if (description != NULL) {
		html_body = apr_psprintf(r->pool, "%s<p>Description: <pre>%s</pre></p>",
				html_body, kap_util_html_escape(r->pool, description));
	}

	return kap_util_html_send(r, "Error", NULL, NULL, html_body, status_code);
}

/*
 * read all bytes from the HTTP request
 */
static apr_byte_t kap_util_read(request_rec *r, const char **rbuf) {
 ...
 */
apr_byte_t kap_util_read_form_encoded_params(request_rec *r,
		apr_table_t *table, const char *data) {
	const char *key, *val, *p = data;

	while (p && *p && (val = ap_getword(r->pool, &p, '&'))) {
		key = ap_getword(r->pool, &val, '=');
		key = kap_util_unescape_string(r, key);
		val = kap_util_unescape_string(r, val);
		apr_table_set(table, key, val);
	}

	kap_debug(r, "parsed: \"%s\" in to %d elements", data,
...
apr_byte_t kap_util_read_post_params(request_rec *r, apr_table_t *table) {
	...
	if (kap_util_read(r, &data) != TRUE)
		return FALSE;

	return kap_util_read_form_encoded_params(r, table, data);
}
...
apr_byte_t kap_util_file_read(request_rec *r, const char *path, char **result) {
	...
	APR_OS_DEFAULT, r->pool)) != APR_SUCCESS) {
		kap_warn(r, "no file found at: \"%s\"", path);
		return FALSE;
	}
	...
	if ((rc = apr_file_info_get(&finfo, APR_FINFO_SIZE, fd)) != APR_SUCCESS) {
		kap_error(r, "error calling apr_file_info_get on file: \"%s\" (%s)",
	...
	*result = (char *)apr_palloc(r->pool, finfo.size + 1);
	...
	if ((rc = apr_file_read_full(fd, *result, finfo.size, &bytes_read))
			!= APR_SUCCESS) {
		kap_error(r, "apr_file_read_full on (%s) returned an error: %s", path,
	...
	kap_debug(r, "file read successfully \"%s\"", path);
 ...
 */
apr_byte_t kap_util_json_array_has_value(request_rec *r, json_t *haystack,
		const char *needle) {
	...
	for (i = 0; i < json_array_size(haystack); i++) {
		...
		if (!json_is_string(elem)) {
			kap_error(r, "unhandled in-array JSON non-string object type [%d]",
					elem->type);
			continue;
		}
 ...
 */
void kap_util_set_app_info(request_rec *r, const char *s_key,
	...
	const char *s_name = apr_psprintf(r->pool, "%s%s", claim_prefix,
			kap_normalize_header_name(r, s_key));
	...
	if (as_header) {
		...
		kap_debug(r, "setting header \"%s: %s\"", s_name, s_value);
	...
	if (as_env_var) {
		...
		/* do some logging about this event */
		kap_debug(r, "setting environment variable \"%s: %s\"", s_name,
				s_value);
		...
		apr_pool_userdata_set(env, KAP_USERDATA_ENV_KEY, NULL, r->pool);
	}
}
 ...
 */
void kap_util_set_app_infos(request_rec *r, const json_t *j_attrs,
	...
	if (j_attrs == NULL) {
		kap_debug(r, "no attributes to set");
		return;
	}
	...
	while (iter) {
		...
		//		kap_util_set_app_info(r, s_key, s_value, claim_prefix);
		//		free(s_value);
		...
		if (json_is_string(j_value)) {
			...
			kap_util_set_app_info(r, s_key, json_string_value(j_value),
			...
			if (sprintf(s_int, "%ld",
				...
				kap_util_set_app_info(r, s_key, s_int, claim_prefix, as_header,
						as_env_var);
			} else {
				kap_warn(r,
			...
			kap_util_set_app_info(r, s_key, s_value, claim_prefix, as_header,
					as_env_var);
			free(s_value);
			...
			for (i = 0; i < json_array_size(j_value); i++) {
				...
				if (json_is_string(elem)) {
					...
					// TODO: escape the delimiter in the values (maybe reuse/extract url-formatted code from kap_session_identity_encode)
			...
			kap_util_set_app_info(r, s_key, s_concat, claim_prefix, as_header,
					as_env_var);
 ...
 */
apr_hash_t *kap_util_spaced_string_to_hashtable(apr_pool_t *pool,
		const char *str) {
	char *val;
 ...
 */
apr_byte_t kap_util_spaced_string_equals(apr_pool_t *pool, const char *a,
		const char *b) {

	/* parse both entries as hash tables */
	apr_hash_t *ht_a = kap_util_spaced_string_to_hashtable(pool, a);
	apr_hash_t *ht_b = kap_util_spaced_string_to_hashtable(pool, b);
 ...
 */
apr_byte_t kap_util_spaced_string_contains(apr_pool_t *pool,
		const char *response_type, const char *match) {
	apr_hash_t *ht = kap_util_spaced_string_to_hashtable(pool, response_type);
 ...
 */
apr_byte_t kap_json_object_get_string(apr_pool_t *pool, json_t *json,
 ...
 */
apr_byte_t kap_json_object_get_int(apr_pool_t *pool, json_t *json,
 ...
 */
void kap_util_table_add_query_encoded_params(apr_pool_t *pool,
 ...
 */
apr_hash_t * kap_util_merge_symmetric_key(apr_pool_t *pool, apr_hash_t *keys,
 ...
 * merge two key sets
 */
apr_hash_t * kap_util_merge_key_sets(apr_pool_t *pool, apr_hash_t *k1,
		apr_hash_t *k2) {
	if (k1 == NULL) {
		if (k2 == NULL)
 ...
 * regexp match
 */
#define KAP_UTIL_REGEXP_MATCH_SIZE 30
#define KAP_UTIL_REGEXP_MATCH_NR 1

apr_byte_t kap_util_regexp_first_match(apr_pool_t *pool, const char *input,
		const char *regexp, char **output, char **error_str) {
	const char *errorptr;
	int erroffset;
	pcre *preg;
	int subStr[KAP_UTIL_REGEXP_MATCH_SIZE];
	...
	if (pcre_get_substring(input, subStr, rc, KAP_UTIL_REGEXP_MATCH_NR,
			&(psubStrMatchStr)) <= 0) {
