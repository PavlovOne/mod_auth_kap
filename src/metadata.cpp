...
#define KAP_METADATA_SUFFIX_PROVIDER "provider"
#define KAP_METADATA_SUFFIX_CLIENT "client"
#define KAP_METADATA_SUFFIX_CONF "conf"

/*
 * get the metadata filename for a specified issuer (cq. urlencode it)
 */
static const char *kap_metadata_issuer_to_filename(request_rec *r,
		const char *issuer) {

	/* strip leading https:// */
	char *p = (char *)strstr(issuer, "https://");
	if (p == issuer) {
		p = apr_pstrdup(r->pool, issuer + strlen("https://"));
	} else {
		p = (char *)strstr(issuer, "http://");
		if (p == issuer) {
	...
	return kap_util_escape_string(r, p);
}

/*
 * get the issuer from a metadata filename (cq. urldecode it)
 */
static const char *kap_metadata_filename_to_issuer(request_rec *r,
		const char *filename) {
	char *result = apr_pstrdup(r->pool, filename);
	char *p = strrchr(result, '.');
	*p = '\0';
	p = kap_util_unescape_string(r, result);
 ...
 */
static const char *kap_metadata_file_path(request_rec *r, kap_cfg *cfg,
		const char *issuer, const char *type) {
	return apr_psprintf(r->pool, "%s/%s.%s", cfg->metadata_dir,
			kap_metadata_issuer_to_filename(r, issuer), type);
}

/*
 * get the full path to the provider metadata file for a specified issuer
 */
static const char *kap_metadata_provider_file_path(request_rec *r,
		const char *issuer) {
	kap_cfg *cfg = (kap_cfg *)ap_get_module_config(r->server->module_config,
			&auth_kap_module);
	return kap_metadata_file_path(r, cfg, issuer,
			KAP_METADATA_SUFFIX_PROVIDER);
}

/*
 * get the full path to the client metadata file for a specified issuer
 */
static const char *kap_metadata_client_file_path(request_rec *r,
		const char *issuer) {
	kap_cfg *cfg = (kap_cfg *)ap_get_module_config(r->server->module_config,
			&auth_kap_module);
	return kap_metadata_file_path(r, cfg, issuer, KAP_METADATA_SUFFIX_CLIENT);
}

/*
 * get the full path to the custom config file for a specified issuer
 */
static const char *kap_metadata_conf_path(request_rec *r, const char *issuer) {
	kap_cfg *cfg = (kap_cfg *)ap_get_module_config(r->server->module_config,
			&auth_kap_module);
	return kap_metadata_file_path(r, cfg, issuer, KAP_METADATA_SUFFIX_CONF);
}

/*
 * get cache key for the JWKs file for a specified URI
 */
static const char *kap_metadata_jwks_cache_key(request_rec *r,
		const char *jwks_uri) {
	return jwks_uri;
}
 ...
 */
static apr_byte_t kap_metadata_file_read_json(request_rec *r, const char *path,
		json_t **result) {
	char *buf = NULL;

	/* read the file contents */
	if (kap_util_file_read(r, path, &buf) == FALSE)
		return FALSE;
	...
	*result = json_loads(buf, 0, &json_error);
	...
	if (*result == NULL) {
		...
		kap_error(r, "JSON parsing (%s) returned an error: %s", path,
				json_error.text);
		return FALSE;
	}
	...
	if (!json_is_object(*result)) {
		/* oops, no JSON */
		kap_error(r, "parsed JSON from (%s) did not contain a JSON object",
				path);
		json_decref(*result);
		return FALSE;
	}
 ...
 */
static apr_byte_t kap_metadata_is_valid_uri(request_rec *r, const char *type,
 ...
 */
static apr_byte_t kap_metadata_provider_is_valid(request_rec *r,
	...
	if (issuer != NULL) {
		if (kap_util_issuer_match(issuer, json_string_value(j_issuer)) == FALSE) {
			kap_warn(r,
	...
	if ((j_response_types_supported != NULL)
		...
		for (i = 0; i < json_array_size(j_response_types_supported); i++) {
			...
			if (kap_proto_flow_is_supported(r->pool, json_string_value(elem)))
				break;
		}
	...
	if (kap_metadata_is_valid_uri(r, "provider", issuer, j_provider,
 ...
 */
static apr_byte_t kap_metadata_client_is_valid(request_rec *r,
	...
	if (apr_time_sec(apr_time_now()) > json_integer_value(expires_at)) {
		kap_warn(r, "client (%s) secret expired", issuer);
		return FALSE;
	}

	kap_debug(r, "client (%s) metadata is valid", issuer);
 ...
 */
static apr_byte_t kap_metadata_jwks_is_valid(request_rec *r,
		const kap_jwks_uri_t *jwks_uri, json_t *j_jwks) {
 ...
 */
static apr_byte_t kap_metadata_conf_jose_is_supported(request_rec *r,
 ...
 */
static apr_byte_t kap_metadata_conf_is_valid(request_rec *r, json_t *j_conf,
		const char *issuer) {

	if (kap_metadata_conf_jose_is_supported(r, j_conf, issuer,
 ...
 */
static apr_byte_t kap_metadata_file_write(request_rec *r, const char *path,
		const char *data) {
	...
	if ((rc = apr_file_open(&fd, path, (APR_FOPEN_WRITE | APR_FOPEN_CREATE),
		...
		kap_error(r, "file \"%s\" could not be opened (%s)", path,
	...
	kap_debug(r, "file \"%s\" written; number of bytes (%" APR_SIZE_T_FMT ")",
			path, len);
 ...
 */
static apr_byte_t kap_metadata_client_register(request_rec *r, kap_cfg *cfg,
		kap_provider_t *provider, json_t **j_client, const char **response) {
	...
	apr_array_header_t *flows = kap_proto_supported_flows(r->pool);
	int i;
	...
	json_object_set_new(data, "logout_uri",
			json_string(
					apr_psprintf(r->pool, "%s?logout=%s", cfg->redirect_uri,
							KAP_GET_STYLE_LOGOUT_PARAM_VALUE)));
	...
	if (provider->registration_endpoint_json != NULL) {
		...
		if (json == NULL) {
			...
			kap_error(r, "JSON parsing returned an error: %s",
					json_error.text);
			...
			if (!json_is_object(json)) {
				...
				kap_error(r, "parsed JSON did not contain a JSON object");

			} else {

				kap_util_json_merge(json, data);
	...
	if (kap_util_http_post_json(r, provider->registration_endpoint_url, data,
	...
	return kap_util_decode_json_and_check_error(r, *response, j_client);
}

/*
 * helper function to get the JWKs for the specified issuer
 */
static apr_byte_t kap_metadata_jwks_retrieve_and_cache(request_rec *r,
		kap_cfg *cfg, const kap_jwks_uri_t *jwks_uri, json_t **j_jwks) {
	...
	if (kap_util_http_get(r, jwks_uri->url, NULL, NULL,
			NULL, jwks_uri->ssl_validate_server, &response, cfg->http_timeout_long,
			cfg->outgoing_proxy, dir_cfg->pass_cookies) == FALSE)
		return FALSE;

	/* decode and see if it is not an error response somehow */
	if (kap_util_decode_json_and_check_error(r, response, j_jwks) == FALSE)
		return FALSE;

	/* check to see if it is valid metadata */
	if (kap_metadata_jwks_is_valid(r, jwks_uri, *j_jwks) == FALSE)
		return FALSE;

	/* store the JWKs in the cache */
	cfg->cache->set(r, KAP_CACHE_SECTION_JWKS,
			kap_metadata_jwks_cache_key(r, jwks_uri->url), response,
 ...
 */
apr_byte_t kap_metadata_jwks_get(request_rec *r, kap_cfg *cfg,
		const kap_jwks_uri_t *jwks_uri, json_t **j_jwks, apr_byte_t *refresh) {

	kap_debug(r, "enter, jwks_uri=%s, refresh=%d", jwks_uri->url, *refresh);

	/* see if we need to do a forced refresh */
	if (*refresh == TRUE) {
		kap_debug(r, "doing a forced refresh of the JWKs from URI \"%s\"",
				jwks_uri->url);
		if (kap_metadata_jwks_retrieve_and_cache(r, cfg, jwks_uri,
				j_jwks) == TRUE)
			return TRUE;
	...
	cfg->cache->get(r, KAP_CACHE_SECTION_JWKS,
			kap_metadata_jwks_cache_key(r, jwks_uri->url), &value);

	if (value == NULL) {
		/* it is non-existing or expired: do a forced refresh */
		*refresh = TRUE;
		return kap_metadata_jwks_retrieve_and_cache(r, cfg, jwks_uri, j_jwks);
	}

	/* decode and see if it is not an error response somehow */
	if (kap_util_decode_json_and_check_error(r, value, j_jwks) == FALSE)
		return FALSE;
 ...
 */
apr_byte_t kap_metadata_provider_retrieve(request_rec *r, kap_cfg *cfg,
	...
	if (kap_util_http_get(r, url, NULL, NULL, NULL,
	...
	/* decode and see if it is not an error response somehow */
	if (kap_util_decode_json_and_check_error(r, *response, j_metadata) == FALSE)
		return FALSE;

	/* check to see if it is valid metadata */
	if (kap_metadata_provider_is_valid(r, *j_metadata, issuer) == FALSE)
		return FALSE;
 ...
 */
static apr_byte_t kap_metadata_provider_get(request_rec *r, kap_cfg *cfg,
	...
	const char *provider_path = kap_metadata_provider_file_path(r, issuer);

	/* see if we have valid metadata already, if so, return it */
	if (kap_metadata_file_read_json(r, provider_path, j_provider) == TRUE) {

		/* return the validation result */
		return kap_metadata_provider_is_valid(r, *j_provider, issuer);
	}
	...
	if (kap_metadata_provider_retrieve(r, cfg, issuer, url, j_provider,
			&response) == FALSE)
		return FALSE;

	/* since it is valid, write the obtained provider metadata file */
	if (kap_metadata_file_write(r, provider_path, response) == FALSE)
		return FALSE;
 ...
 */
static apr_byte_t kap_metadata_conf_get(request_rec *r, kap_cfg *cfg,
		const char *issuer, json_t **j_conf) {

	/* get the full file path to the conf metadata for this issuer */
	const char *conf_path = kap_metadata_conf_path(r, issuer);
	...
	/* see if we have valid metadata already, if so, return it */
	if (kap_metadata_file_read_json(r, conf_path, j_conf) == TRUE) {

		/* return the validation result */
		return kap_metadata_conf_is_valid(r, *j_conf, issuer);
	}
 ...
 */
static apr_byte_t kap_metadata_client_get(request_rec *r, kap_cfg *cfg,
		const char *issuer, kap_provider_t *provider, json_t **j_client) {

	/* get the full file path to the client metadata for this issuer */
	const char *client_path = kap_metadata_client_file_path(r, issuer);

	/* see if we have valid metadata already, if so, return it */
	if (kap_metadata_file_read_json(r, client_path, j_client) == TRUE) {

		/* if the client metadata is (still) valid, return it */
		if (kap_metadata_client_is_valid(r, *j_client, issuer) == TRUE)
			return TRUE;
	}
	...
	const char *response = NULL;
	if (kap_metadata_client_register(r, cfg, provider, j_client,
			&response) == FALSE)
		return FALSE;

	/* check to see if it is valid metadata */
	if (kap_metadata_client_is_valid(r, *j_client, issuer) == FALSE)
		return FALSE;

	/* since it is valid, write the obtained client metadata file */
	if (kap_metadata_file_write(r, client_path, response) == FALSE)
		return FALSE;
...
/*
 * get a list of configured KAP providers based on the entries in the provider metadata directory
 */
apr_byte_t kap_metadata_list(request_rec *r, kap_cfg *cfg,
	...
	if ((rc = apr_dir_open(&dir, cfg->metadata_dir, r->pool)) != APR_SUCCESS) {
		kap_error(r, "error opening metadata directory '%s' (%s)",
	...
	*list = apr_array_make(r->pool, 5, sizeof(sizeof(const char*)));
	...
	while (apr_dir_read(&fi, APR_FINFO_NAME, dir) == APR_SUCCESS) {
		...
		char *ext = (char *)strrchr(fi.name, '.');
		if ((ext == NULL)
				|| (strcmp(++ext, KAP_METADATA_SUFFIX_PROVIDER) != 0))
			continue;

		/* get the issuer from the filename */
		const char *issuer = kap_metadata_filename_to_issuer(r, fi.name);

		/* get the provider and client metadata, do all checks and registration if possible */
		kap_provider_t *provider = NULL;
		if (kap_metadata_get(r, cfg, issuer, &provider) == TRUE) {
...
/*
 * parse the JSON provider metadata in to a kap_provider_t struct but do not override values already set
 */
apr_byte_t kap_metadata_provider_parse(request_rec *r, json_t *j_provider,
		kap_provider_t *provider) {

	if (provider->issuer == NULL) {
		/* get the "issuer" from the provider metadata */
		kap_json_object_get_string(r->pool, j_provider, "issuer",
				&provider->issuer, NULL);
	}

	if (provider->authorization_endpoint_url == NULL) {
		/* get a handle to the authorization endpoint */
		kap_json_object_get_string(r->pool, j_provider,
	...
	if (provider->check_session_iframe == NULL) {
		...
		kap_json_object_get_string(r->pool, j_provider, "check_session_iframe",
				&provider->check_session_iframe, NULL);
	}

	if (provider->end_session_endpoint == NULL) {
		/* get a handle to the end session endpoint */
		kap_json_object_get_string(r->pool, j_provider, "end_session_endpoint",
...
/*
 * parse the JSON conf metadata in to a kap_provider_t struct
 */
apr_byte_t kap_metadata_conf_parse(request_rec *r, kap_cfg *cfg,
		json_t *j_conf, kap_provider_t *provider) {

	kap_json_object_get_string(r->pool, j_conf, "client_jwks_uri",
			&provider->client_jwks_uri, cfg->provider.client_jwks_uri);

	kap_json_object_get_string(r->pool, j_conf, "id_token_signed_response_alg",
			&provider->id_token_signed_response_alg,
			cfg->provider.id_token_signed_response_alg);
	kap_json_object_get_string(r->pool, j_conf,
			"id_token_encrypted_response_alg",
	...
	kap_json_object_get_string(r->pool, j_conf, "userinfo_signed_response_alg",
	...
	kap_json_object_get_int(r->pool, j_conf, "ssl_validate_server",
...
/*
 * parse the JSON client metadata in to a kap_provider_t struct
 */
apr_byte_t kap_metadata_client_parse(request_rec *r, kap_cfg *cfg,
		json_t *j_client, kap_provider_t *provider) {

	/* get a handle to the client_id we need to use for this provider */
	kap_json_object_get_string(r->pool, j_client, "client_id",
			&provider->client_id, NULL);

	/* get a handle to the client_secret we need to use for this provider */
	kap_json_object_get_string(r->pool, j_client, "client_secret",
	...
	kap_json_object_get_string(r->pool, j_client, "token_endpoint_auth_method",
	...
	if (provider->response_type == NULL) {
		...
		if ((j_response_types != NULL) && (json_is_array(j_response_types))) {
			...
			if (kap_util_json_array_has_value(r, j_response_types,
 ...
 *
 * this fill the kap_provider_t struct based on the issuer filename by reading and merging
 * contents from both provider metadata directory and client metadata directory
 */
apr_byte_t kap_metadata_get(request_rec *r, kap_cfg *cfg, const char *issuer,
		kap_provider_t **provider) {
	...
	*provider = (kap_provider_t *)apr_pcalloc(r->pool, sizeof(kap_provider_t));
	...
	if (kap_metadata_provider_get(r, cfg, issuer, &j_provider) == FALSE)
		goto end;
	if (kap_metadata_provider_parse(r, j_provider, *provider) == FALSE)
		goto end;

	if (kap_metadata_conf_get(r, cfg, issuer, &j_conf) == FALSE)
		goto end;
	if (kap_metadata_conf_parse(r, cfg, j_conf, *provider) == FALSE)
		goto end;

	if (kap_metadata_client_get(r, cfg, issuer, *provider, &j_client) == FALSE)
		goto end;
	if (kap_metadata_client_parse(r, cfg, j_client, *provider) == FALSE)
		goto end;
